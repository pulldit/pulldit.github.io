...
- [x] 3.3 `index.html` — collapsible History panel + Clear button + list
- [x] 3.4 `styles.css` — history list styles
- [x] 3.5 `src/app.js` — load/save/render history; record fetch, download, zip,
        discard, restore events; wire Clear button

## Phase 4 — Make statistics always visible
- [x] 4.1 `index.html` — fetch-stats panel always visible (remove `hidden`)
- [x] 4.2 `src/app.js` — render an idle/empty fetch-stats state on init
- [x] 4.3 `index.html` — cache-bust entry assets (styles.css, src/app.js `?v=`) so
